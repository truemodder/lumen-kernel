		...
		if (posStart) {
			...
			if (posEnd) {
				...
				if (type < DSI_CMD_SET_MAX) {
					dsi_panel_destroy_cmd_packets_mot(&priv_info->cmd_sets[type]);
					dsi_panel_dealloc_cmd_packets_mot(&priv_info->cmd_sets[type]);
					dsi_panel_alloc_cmd_packets_mot(&priv_info->cmd_sets[type], cmdlen);
					dsi_panel_create_cmd_packets_mot(data, 0, cmdlen, priv_info->cmd_sets[type].cmds);
				}
